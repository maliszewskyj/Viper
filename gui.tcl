...
proc ScalerStatusBuild { p } {
    ...
    if {$p == "."} {
	set pf [frame .panel -borderwidth 2 -relief ridge ]
    } else {
	set pf [frame $p.panel -borderwidth 2 -relief ridge]
    }

    label $pf.armed -text Armed -background DarkRed -relief raised \
	    -borderwidth 2 
    set sc(armed) $pf.armed
    label $pf.modelab -text "Mode:" 
    label $pf.mode -textvariable sc(mode) \
	    -width 12 -relief sunken -background white 
    label $pf.preslab -text "Preset:" 
    label $pf.preset -textvariable sc(preset) \
	    -width 12 -relief sunken -background white -anchor e
    label $pf.nprslab -text "N Presets:" 
    label $pf.nprset -textvariable sc(npreset) \
	    -width 12 -relief sunken -background white -anchor e
    label $pf.timelab -text "Time:" 
    label $pf.time -textvariable sc(tottime) \
	    -width 12 -relief sunken -background white -anchor e
    label $pf.monlab  -text "Monitor:" 
    label $pf.mon  -textvariable sc(monitor) \
	    -width 12 -relief sunken -background white -anchor e
    label $pf.detlab  -text "Det 1:"
    label $pf.det  -textvariable sc(detector) \
	    -width 12 -relief sunken -background white -anchor e    
    label $pf.totlab  -text "Total:" 
    label $pf.total -textvariable sc(total) \
	    -width 12 -relief sunken -background white -anchor e

    grid $pf.armed   -          -in $pf -sticky ew
    grid $pf.modelab $pf.mode   -in $pf -sticky w
    grid $pf.preslab $pf.preset -in $pf -sticky w
    grid $pf.nprslab $pf.nprset -in $pf -sticky w
    grid $pf.timelab $pf.time   -in $pf -sticky w
    grid $pf.monlab  $pf.mon    -in $pf -sticky w
    grid $pf.detlab  $pf.det    -in $pf -sticky w
    grid $pf.totlab  $pf.total  -in $pf -sticky w
...
proc ScalerGraphBuild { p } {
	...
	$pg xaxis configure -title {Detector} -stepsize 10
	if {[info exists sc(xmin)] && [info exists sc(xmax)]} {
	    $pg xaxis configure -min $sc(xmin) -max $sc(xmax)
	}
	set sc(ymin) 0
	set sc(ymax) 1
	$pg yaxis configure -title {Counts}
	$pg element configure e0 -xdata $xdata -ydata $ydata
	$pg yaxis configure -min $sc(ymin) -max $sc(ymax)
	$pg legend configure -hide true
    }
...
proc MotorAxisControl { axis args } {
    ...
    # Fix packing
    catch {pack forget $display(graph)}
    catch {pack forget $display(scaler)}
    #catch {pack forget $display(motor)}
    catch {pack forget $display(control)}
    #pack $display(motor) -side left -fill y
    pack $display(control) -side left -anchor nw 
...
proc MotorAxisControlBuild { p } {
    ...
    set f [frame $pf -borderwidth 2 -relief ridge]
    ...
    set mcgui(pos) [label $f.pos -textvariable mc(1,position)\
	    -relief sunken -background white -width 12]
    frame $f.stafrm
    set mcgui(sta) [label $f.stafrm.sta -textvariable mc(1,status)\
	    -relief sunken -background white -width 10]
    set mcgui(neglim) [label $f.stafrm.neglim -text "-" -width 1 -relief sunken -background white]
    set mcgui(poslim) [label $f.stafrm.poslim -text "+" -width 1 -relief sunken -background white]
    grid $f.stafrm.neglim $f.stafrm.sta $f.stafrm.poslim -in $f.stafrm
    ...
    grid $f.stalab $f.stafrm $f.set  -in $f -sticky ew
...
proc MotorAxisStat { p axis } {
    ...
    set mcgui($axis,neglim) [label $pf.neglim -text "-" -width 1 -relief sunken -background white]
    set mcgui($axis,poslim) [label $pf.poslim -text "+" -width 1 -relief sunken -background white]
    # pack $pf.control $pf.pos $pf.stat $pf.desc -side left -in $pf
    pack $pf.control $pf.neglim $pf.pos $pf.poslim $pf.stat $pf.desc -side left -in $pf
    return $pf
} 
...
proc MotorStatusPanelUpdate { axis } {
    ...
    if [info exists mcgui($axis,poslim)] {
	if {$mc($axis,limstat) & 0x2} {
	    set pcolor "red"
	} else {
	    set pcolor "white"
	}
	$mcgui($axis,poslim) configure -background $pcolor
	if {$iscurrent} { $mcgui(poslim) configure -background $pcolor }
    }

    if [info exists mcgui($axis,neglim)] {
	if {$mc($axis,limstat) & 0x1} {
	    set ncolor "red"
	} else {
	    set ncolor "white"
	}
	$mcgui($axis,neglim) configure -background $ncolor
	if {$iscurrent} { $mcgui(neglim) configure -background $ncolor }
    }
...
proc MotorStatPanel { p } {
    ...
    if {$p == "."} {
	#set pm [frame .m -borderwidth 2 -relief ridge]
	set pm [ScrolledWindow .m]      ;# BWidget
	set pmf [ScrollableFrame .m.sf] ;# BWidget
	$pm setwidget $pmf
	set pf [$pmf getframe]
    } else {
	set pm [ScrolledWindow $p.m]      ;# BWidget
	set pmf [ScrollableFrame $p.m.sf] ;# BWidget
	$pm setwidget $pmf
	set pf [$pmf getframe]
    }

    foreach m $mc(defined) {
	set pma [MotorAxisStat $pf $m]
	pack $pma -in $pf -side top
    }
...
proc RelayPanel { p } {
    ...
    if {$p == "."} {
	set f [frame .relay -borderwidth 2 -relief ridge]
    } else {
...
proc DioPanel { p } {
    ...
    if {$p == "."} {
	set f [frame .dio -borderwidth 2 -relief ridge]
    } else {
...
proc CommStatPanel { p } {
    ...
    if {$display(havescaler) || $display(havedio)} {
...
proc DisplayCycle { } {
    ...
    if {![info exists display(scaler)]} {incr display(current)}
    if {$display(current) > $display(max)} { set display(current) 0 }
    puts "Display(current) = $display(current)"
    set geom $display(geometry)
    switch $display(current) {
	0 {
	    if [info exists display(scaler)] {
		pack forget $display(scaler)
	    }
	    pack forget $display(edit) $display(control) 
	    ...
	    if [info exists display(scaler)] {
		pack $display(motor) -side left -anchor nw -fill y
		pack $display(scaler) -side left -anchor nw \
		    -in $display(base)
	...
	1 {
	    ...
	    pack forget $display(motor) $display(control)
	...
	2 {
	    ...
	    pack forget $display(motor) $display(control) $display(edit)
...
proc BuildGUI { } {
	...
	set ps [ScalerStatusBuild .]
	set display(scaler) $ps
    }

    if {$sc(plotdata)} {
	set pg [ScalerGraphBuild .]
    ...
    set pm [MotorStatPanel .]
    set pa [MotorAxisControlBuild .]
    set pc [CommStatPanel .]

    pack $pc -side bottom -anchor sw -fill x 
    pack $pm -side left -anchor nw -fill y
    if {$display(havescaler)} {
	pack $ps -side left -anchor nw 
    } 
    ...
    set display(max)     1
    if {$display(havescaler)} { incr display(max) }
    if {$display(haverelay) || $display(havedio)}  { incr display(max)}
